...
void PauseButton::on_module_loaded(){
    ...
    this->button     =  this->kernel->config->value( pause_button_pin_checksum )->by_default(PAUSE_BUTTON_PIN)->as_pin()->as_input();
    this->led        =  this->kernel->config->value( pause_led_pin_checksum    )->by_default(PAUSE_LED_PIN)->as_pin()->as_output();
