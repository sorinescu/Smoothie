...
smt_string SimpleShell::absolute_from_relative( smt_string path ){
...
void SimpleShell::ls_command( smt_string parameters, StreamOutput* stream ){
//    smt_string folder = this->absolute_from_relative( parameters );
//    DIR* d;
//    struct dirent* p;
//    d = opendir(folder.c_str());
//    if(d != NULL) {
//        while((p = readdir(d)) != NULL) { stream->printf("%s\r\n", lc(smt_string(p->d_name)).c_str()); }
//    } else {
...
void SimpleShell::cd_command( smt_string parameters, StreamOutput* stream ){
//    smt_string folder = this->absolute_from_relative( parameters );
...
void SimpleShell::pwd_command( smt_string parameters, StreamOutput* stream ){
...
void SimpleShell::cat_command( smt_string parameters, StreamOutput* stream ){


    // // Get parameters ( filename and line limit )
    // smt_string filename          = this->absolute_from_relative(shift_parameter( parameters ));
    // smt_string limit_paramater   = shift_parameter( parameters );
    // int limit = -1;
...
void SimpleShell::play_command( smt_string parameters, StreamOutput* stream ){
...
// Reset the system
void SimpleShell::reset_command( smt_string parameters, StreamOutput* stream){
