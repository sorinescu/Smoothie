...
CartesianSolution::CartesianSolution(Config* passed_config) : config(passed_config){
    ...
    this->alpha_steps_per_mm = this->config->value( alpha_steps_per_mm_checksum)->by_default(5)->as_number();
    this->beta_steps_per_mm  = this->config->value(  beta_steps_per_mm_checksum)->by_default(5)->as_number();
    this->gamma_steps_per_mm = this->config->value( gamma_steps_per_mm_checksum)->by_default(5)->as_number();
}
