...
smt_vector<uint16_t>::type get_checksums(smt_string key){
    ...
    while( begin_key < key.size()-1 ){
        ...
        smt_string key_node = key.substr(begin_key, end_key - begin_key);
...
smt_string remove_non_number( smt_string str ){
    smt_string number_mask = "0123456789-.";
    size_t found=str.find_first_not_of(number_mask);
    while (found!=smt_string::npos){
        //str[found]='*';
        str.replace(found,1,"");
...
// Get the first parameter, and remove it from the original smt_string
smt_string shift_parameter( smt_string &parameters ){
    size_t beginning = parameters.find_first_of(" ");
    if( beginning == smt_string::npos ){ smt_string temp = parameters; parameters = ""; return temp; }
    smt_string temp = parameters.substr( 0, beginning );
...
smt_string get_arguments( smt_string possible_command ){
    ...
    if( beginning == smt_string::npos ){ return ""; }
